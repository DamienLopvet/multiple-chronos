  ...
  <section>
    <header :class="{ dark: darkTheme }">
      ...
      <audio controls="" name="media">
        <source src="https://icecast.radiofrance.fr/fip-hifi.aac" type="audio/aac" />
      </audio>
    ...
    <div class="chronos-container">
      <div class="chrono-card" v-for="(chrono, index) in arrayOfChronos" :key="index">
        <!-- Title -->
        <div class="chrono-title" @click="chrono.showEditTitle = true" :style="{ 'background-color': chrono.color }">
          <p type="text">{{ chrono.title }}</p>
          <i class="fa-solid fa-pen chrono-title-edit cursor-pointer" @click="chrono.showEditTitle = true"></i>
        </div>
        <!-- counter -->
        ...
        <div class="chrono-nav" :style="{ 'background-color': chrono.color }">
          ...
          <!-- forward -->
          <span v-if="chrono.starTime > 0" class="chrono-forward cursor-pointer" @mousedown.left="forward(chrono)"
            @mouseup.left="stopForwarding()" @touchstart="forward(chrono)" @touchend="stopForwarding()"><i
    ...
    <div :class="{ dark: darkTheme }">
      <span v-if="darkTheme" class="theme cursor-pointer" @click="(darkTheme = !darkTheme)"
        :class="{ dark: darkTheme }"><i class="fa-solid fa-sun" :class="{ dark: darkTheme }"></i></span>
      <span v-else class="theme cursor-pointer" @click="(darkTheme = !darkTheme)"><i class="fa-solid fa-moon"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      sendToLocalStorage() {
        localStorage.setItem("chronos", JSON.stringify(this.arrayOfChronos));
      },
      /***GET LOCAL STORAGE ITEMS**/
      getLocalStorage() {
        this.arrayOfChronos = JSON.parse(localStorage.getItem("chronos")) || [];
      ...
      method: "get",
      url: `http://localhost:3000/api/chronos`,
      ...
      randomColor() {
        ...
        let color =
          this.colors[Math.floor(Math.random() * (max - min + 1) + min)];
        return color;
      },
      createChrono() {
        let chronoToAdd = {
    ...
    created(){
      console.log(this.tokenIsSet);
      console.log(this.userIsSet);
